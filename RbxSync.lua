...
local moonBoilerplate = [==[-- RbxSync Boilerplate --
...
local pmPath = "Documents\\ROBLOX\\RbxSync"
local scriptCache = { }
local sourceCache = { }
...
local debug
debug = function(...)
  return print("[RbxSync build " .. tostring(BUILD) .. "] ", ...)
end
local alert
alert = function(...)
  debug(...)
...
checkForPlaceName = function(obj)
  if not (obj) then
    if HttpService:FindFirstChild("PlaceName") then
  ...
  if obj:IsA("StringValue") and #obj.Value > 0 then
    resetCache()
    gameGUID = obj.Value
    temp = false
    return scan()
...
if game.Name:match("Place[%d+]") and (game:GetService("RunService"):IsClient() and game:GetService("RunService"):IsServer()) then
  ...
  checkForPlaceName()
  return HttpService.ChildAdded:connect(checkForPlaceName)
end
